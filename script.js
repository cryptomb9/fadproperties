...
function shortText(value, limit = 118) {
  ...
  if (text.length <= limit) return text || "Speak with FAD Home and Properties for full details, inspection notes, and availability.";
...
function renderShowcase(properties = []) {
  ...
  const propertySlides = properties
    ...
    .map((item) => ({
      ...
      text: item.description || "Listed and managed by FAD Home and Properties.",
    }));
  ...
  const fallbackSlides = [
    {
      ...
      text: "A finished contemporary home built for the kind of lifestyle and quality FAD Home and Properties represents.",
    },
    {
...
async function loadPromoPopup() {
  ...
  overlay.innerHTML = `
    <div class="modal-content" style="padding:10px;max-width:720px;">
      ...
      <img src="${escapeHtml(promo.image_url)}" alt="FAD Home and Properties promotion" style="width:100%;border-radius:6px;">
    </div>
  `;
